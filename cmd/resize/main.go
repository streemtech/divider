...
func main() {
	...
	d1 := redisconsistent.NewDivider(r, "con:test", uuid.New().String(), l, time.Second, time.Second, 3)
	if d1 == nil {
		panic("Unable to create divider")
	}

	d2 := redisconsistent.NewDivider(r, "con:test", uuid.New().String(), l, time.Second, time.Second, 3)
	if d2 == nil {
		panic("Unable to create divider")
	}
